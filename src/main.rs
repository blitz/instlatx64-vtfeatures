...
{
    lazy_static! {
	static ref RE: Regex = Regex::new(r"^MSR ([0-9a-fA-F]+): ([-0-9a-fA-F]+)$").expect("a valid regex");
    }
	...
	.map(|c|
		 ...
		 value: u64::from_str_radix(&c.get(2).unwrap().as_str()
					    .chars()
					    .filter(|&c| c != '-')
					    .collect::<String>(), 16).ok()?
	     }))
	.flatten()
}
...
fn has_feature(msr_values: &[MsrValue], feature: &Feature) -> Option<bool>
{
	...
	.fold(Some(true), |acc, n| acc.and_then(|b| match n {
	    Some(c) => Some(b && c),
	    None => None
	}))
}
...
fn main() -> Result<()> {
	...
	println!("{}: {}", feature.name, tristate_to_char(has_feature(&msr_values, feature)));
    }
